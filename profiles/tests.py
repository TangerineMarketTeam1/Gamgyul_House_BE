...
class ProfileViewsTestCase(APITestCase):
    ...
    def test_profile_detail_view(self):
        url = reverse("profile_detail", kwargs={"id": str(self.user2.id)})
    ...
    def test_privacy_settings_view_get(self):
        url = reverse("privacy_settings", kwargs={"user_id": str(self.user1.id)})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_privacy_settings_permission_denied(self):
        url = reverse("privacy_settings", kwargs={"user_id": str(self.user2.id)})
        response = self.client.get(url)
